...
def compute_position_in_frame(p, frame):
    ...
    R = np.array([[np.cos(theta_f), np.sin(theta_f)],
                  [-np.sin(theta_f), np.cos(theta_f)]])
    
    p = np.matmul(R,(p[:2] - frame[:2]))
    return p
    # END SOLUTION
...
class BaseController(object):
    ...
    def get_reference_index(self, pose, path_xytv, distance_lookahead):
        ...
        with self.path_condition:
            ...
            path_x = path_xytv[:,0]
            path_y = path_xytv[:,1]
            path_theta = path_xytv[:,2]

            diff = ((x-path_x[:])**2 + (y-path_y[:])**2)**0.5 # scalar vector

            min_ind = np.argmin(diff)
            i = min_ind
            while (diff[i] < distance_lookahead and i < len(path_xytv) -1): #hopefully this is only a few iterations...
                i = i+1
